...
```{r po sim time}
          ...
          splitLayout(
          textInput("p1s",    div(h5(tags$span(style="color:blue", "hyp. proportion(s) \U1D7B9 1...n"))),    
          value= "0.044, 0.019, 0.471, 0.317, 0.149"),)
          ...
          splitLayout(
          textInput("alphas", div(h5(tags$span(style="color:blue", "\U1D7AA (type I errors)"))), 
          value= "0.025"),  )
          
          splitLayout(
          textInput("Qs",    div(h5(tags$span(style="color:blue", "ratios for unequal randomisation"))),   
          value= "0.5"),)          
          ...
          # sliderInput('x_rangepos', 'Plot x-range', value = c(0,800),
          #         min = 0, max = 5000,step=100,ticks=F)
          # 
          # sliderInput('y_rangepos', 'Plot y-range', value = c(0,1),
          #         min = 0, max = 1,step=.05,ticks=F)
...
#------------------------------------------------------------------------------------------------------------  
          ...
          #--------------------------------------------------------------------------
          # input probabilities, total sample size, trt effect on log scale
        
          POsim <- function(P, n, beta_trt) 
            
            {
              ...
              trt <- sample(c(0, 1), n, replace=TRUE)   # treatment indicator
           ...
           #--------------------------------------------------------------------------
           out <- replicate(ss, POsim(P =P, n=N, beta_trt=log(or) ) )
           pvalue <- mean( out <= sig.level )  #  
           #-------------------------------------------------------------------------
           
           
           
           harrell <-  Hmisc::popower(p=P, odds.ratio=or,  n=N,   alpha=sig.level )[[1]]
