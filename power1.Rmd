...
```{r}
          ...
          splitLayout(
          textInput("romx",    
                    div(h5(tags$span(style="color:blue", "n2/n2 ratio for randomisation"))),   
          value= "1"),)     
...
#apply function to each row    
  ...
  for( i in 1: dim(d)[1]) {
    ...
    # circumventing weird results,  function needs mu order for 1 side
...
renderPlot({
    ...
    w <- pomodm(kx, kp, odds.ratio=1) #original mean and median
...
**Enter 3 means or proportions (Ctrl, Trt1, Trt2) and select 1 or 2 for binary or continuous, Bonferroni adjusted**
...
```{r threearm12, eval=TRUE}
          ...
          splitLayout(
          textInput("alphas2", div(h5(tags$span(style="color:blue", "\U1D7AA (type I error) 2 sided"))), 
          value= "0.05"),  )
          ...
          sliderInput('x_rangepo2', 'Plot x-range', value = c(0,80),
                  min = 0, max = 5000,step=20,ticks=F)
...
```
Normal approximation used for proportions
```{r}
...
#---------------------------------------------------------------------------------------------------
 ...
 #---------------------------------------------------------------------------------------------------
                ...
                # function to execute three.arm.v2 over range of N
    #apply function to each row    
   ...
   for( i in 1: length(n)) {
        ...
        names(result)  <-c("Trt1 v Ctrl", "Trt2 v Ctrl", "Both v Ctrl", "Conjunctive (power to reject all 3 null hypotheses)", "Disjunctive (power to reject at least one of 3 null hypotheses)", 
                           "Power to reject at least one of the 2 comparisons to ctrl", "mu1", "mu2", "mu3", "sigma0", "sigma1", "sigma2", "alpha", "out.var", "n")
...
renderPlot({
 ...
 L <- melt(p, id="n", 
           measure.vars=c("Trt1 v Ctrl", "Trt2 v Ctrl", "Both v Ctrl", "Conjunctive (power to reject all 3 null hypotheses)", "Disjunctive (power to reject at least one of 3 null hypotheses)",  "Power to reject at least one of the 2 comparisons to ctrl") )
 
 
  #-----------------------------------------------------------------------
  #-----------------------------------------------------------------------
  skips <- ifelse(input$x_rangepo2[2] <= 500, 10,
           ifelse(input$x_rangepo2[2] > 500 & input$x_rangepo2[2] < 3000 ,200
                       ,  500))
  ...
  labs(y="Power", x = 'Sample Size in each arm', color = "Scenario")+
 
  labs(caption = paste("- Alpha the type I assertion probability." )) +

  ggtitle(paste0(" Power (or sample size) for three arms ") )
...
```{r threearm1}
          ...
          splitLayout(
          textInput("alphas", div(h5(tags$span(style="color:blue", "\U1D7AA (type I error) 2 sided"))), 
          value= "0.05"),  )
                          ...
                          nSims=sims , out.var=1 )
{
  ...
  return (c( 
    "Trt1 v Ctrl",   formatz4(P1),
    "Trt2 v Ctrl",formatz4(P2),
    "Both v Ctrl", formatz4(P3) ,
    "Conjunctive (power to reject all 3 null hypotheses)", formatz4(P4) ,
    "Disjunctive (power to reject at least one of 3 null hypotheses)", formatz4(P5) ,
    "Power to reject at least one of the 2 comparisons to ctrl", formatz4(P6) 
  ))
}
...
# if you uncode this you get repeat of figure on bottom half of simulation text tab
...
#  
#  L <- melt(p, id="n", 
#            measure.vars=c("Trt1 v Ctrl", "Trt2 v Ctrl", "Both v Ctrl", "Conjunctive (power to reject all 3 null hypotheses)", 
#                           "Disjunctive (power to reject at least one of 3 null hypotheses)", 
# "Power to reject at least one of the 2 comparisons to ctrl") )
#  
#    
#  
#  
...
**Enter 2 equal worst response rates separated by commas and best response rate, 3 groups only**
