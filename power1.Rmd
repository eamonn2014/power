...
**Enter a best response rate and remaining 2 as equal worst response rates separated by commas**
...
```{r ptw inputs}
    ...
    #pick.winnner(nSims=1e7,  mu=c(0.2, 0.3, 0.35), nStg1=279)
          ...
          sliderInput('x_ranges_ptw', 'Plot x-range', value = c(0,140),
                  min = 0, max = 1000,step=20,ticks=F)
...
####
#as.vector(unlist(pick.winner(nSims=1e3,  mu=c(0.2, 0.3, 0.35), nStg1=279)))
...
#apply function to each row    
  ...
  for( i in 1: dim(d)[1]) {
           ...
           #d[i,] <-  pick.winner(nSims=s,  mu=h0, nStg1=n[i])
           
            d[i,] <-    as.vector(unlist(pick.winner(nSims=s,  mu=h0, nStg1=n[i])))
            #d[i,] <-    as.vector(unlist(pick.winner(nSims=s,  mu=h0#)))
            #d[i,] <-  runif(8)
