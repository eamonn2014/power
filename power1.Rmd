...
```{r bern}
          ...
          splitLayout(
          textInput("p1_h0",    div(h5(tags$span(style="color:blue", "null hypothesis"))),    
          value= ".37"),)

          splitLayout(
          textInput("p1_h1",    div(h5(tags$span(style="color:blue", "alternative hypothesis"))),    
          value= ".68"),)
           
          splitLayout(
          textInput("alpha_bern", div(h5(tags$span(style="color:blue", "\U1D7AA (type I error)"))), 
          value= "0.025"),  )
          
          splitLayout(
          textInput("target", div(h5(tags$span(style="color:blue", "target at which check power"))), 
          value= "0.8"),  )
...
#------------------------------------------------------------------------------------------------------------  
        ...
        d <- tidyr::expand_grid(h0=h0,  h1=h1, alpha= sig.level, target=target,  n=n )
    
        d$critical1 <-   with(d,qbinom(p = 1 - alpha, size = n, prob = h0))
        d$power <- 1-  with(d,pbinom(critical1, n, p=h1))   # 1- beta
 
        d$optimal <- (d$power > d$target)
        
        
        
        d$phrase <- with(d, paste0("H0=",h0,", H1=",h1," \U1D7AA=",alpha ))
     
        
        
 
        # get cordinates of first time powerr is reached.
        library(dplyr)
         coord <-  d %>%
...
renderPlot({
  ...
  #-----------------------------------------------------------------------
  skips <- ifelse(input$x_ranges_bern[2] <= 1000, 50,
           ifelse(input$x_ranges_bern[2] > 1000 & input$x_ranges_bern[2] < 3000 ,200,  500))
