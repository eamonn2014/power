---
title: "Power or sample size various endpoints" 
author: 
output: 
...
print(paste0(   
 ...
 # tags$a(href = "https://github.com/eamonn2014/power/blob/main/papers%20for%20git/Whitehead%201998%20sample%20size%20review%20in%20a%20head%20injury%20trial.pdf", 
 #            target="_blank",
 #        tags$span(style="color:blue", "SAMPLE SIZE REVIEW IN A HEAD INJURY TRIAL WITH ORDERED CATEGORICAL RESPONSES, WHITHEAD 1998"),) ,
...
# proportional effect of treatment across the scale of the ordinal outcome
...
3 arm trial (continuous and binary sim. with fig.)
===
...
3 arm trial (continuous and binary sim.)
===
...
```{r bern}
          ...
          splitLayout(
          textInput("p1_h0",    
          div(h5(tags$span(style="color:blue", "null hypothesis"))),    
          value= "0.37, 0.50, 0.55"),)

          splitLayout(
          textInput("p1_h1",    
          div(h5(tags$span(style="color:blue", "alternative hypothesis"))),    
          value= ".68"),)
          ...
          splitLayout(
          textInput("target", 
          div(h5(tags$span(style="color:blue", "target at which check power"))), 
          value= "0.8, 0.9"),  )
           
          sliderInput('x_ranges_bern', 'Plot x-range', value = c(0,140),
 ...
 #------------------------------------------------------------------------------------------------------------  
       ...
       #  target=c(.8,.9)
        ...
        # work on finding the numerators
        ...
        d <- plyr::arrange(d, group)
         ...
         PP1 <-  PP %>%
          ...
          # second slice as ther may be multiple non consecutive gaps 
           PP2 <- PP2%>%
           group_by(group) %>% 
             slice_max(b, n = 1)  # max rather than min (to ensure we dont pick another incidence after which power drops below our target)
       ...
       # coord$colr <- ifelse(coord$conservative %in% 1 ,"blue","black")
...
#         
#     
#         # d$critical1 <-   with(d,qbinom(p = 1 - alpha, size = n, prob = h0))
#         # d$power   <- 1-  with(d,pbinom(critical1,     size = n, prob = h1))   # 1- beta
#         # 
#         #d$optimal <- (d$power > d$target)
#         x$phrase <- with(x, paste0("H0=",x$p0,", H1=",x$p1,", \U1D7AA=",alpha ))
#     
#         
# conT <- function() nBinomial1Sample(p0 = p0, p1 =p1, alpha = alpha, beta = .2, n = 1:10000, conservative = TRUE)
# conF <- function() nBinomial1Sample(p0 = p0, p1 =p1, alpha = alpha, beta = .2, n = 1:10000, conservative = FALSE)
#         
#         
#         
#         nBinomial1Sample(p0 = 0.5, p1 = 0.6, alpha = 0.05, beta = .2, n = 1:10000, conservative = FALSE)
# 
#         
#         require(tidyverse)
#        x %>%
#           group_by(p0,p1,alpha,beta) %>%
#           group_map(~ conF())
#                 
#         
#            
#         
#         # get when power is reached/exceeded.
#         library(dplyr)
#          coord <-  d %>%
#            group_by(h0,h1,alpha,target) %>% 
#             filter(optimal == TRUE) %>%
#             slice(consist.)
...
```{r listings bern}
 ...
 DT::renderDataTable({
    ...
    p$group <- p$Attain <- p$conservative <- NULL
