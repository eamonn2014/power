...
Binary endpoint, non inferiority under construction
===
...
```{r tabn}
          ...
          value= "0.1,0.1,0.1,0.25,0.3,0.4"),)
              ...
              splitLayout(
          textInput("ni", div(h5(tags$span(style="color:blue", "non inferiority margin"))), 
          value= "0.05, 0.05,0.05, 0.05,0.05, 0.05"), 
                                       )
          ...
          sliderInput('x_rangen', 'Plot x-range', value = c(0,800),
                  min = 0, max = 5000,step=100,ticks=F)
          
          sliderInput('y_rangen', 'Plot y-range', value = c(0,1),
                  min = 0, max = 1,step=.05,ticks=F)
...
xxni <- reactive({
    ...
    Q <-   (as.numeric(unlist(strsplit(input$p2,","))))    
    R <-   (as.numeric(unlist(strsplit(input$ro,","))))    
    NI <-   (as.numeric(unlist(strsplit(input$ni,","))))   
    ...
    for (i in 1:6) {
          ...
          assign(paste0("A", i), A[i])
          assign(paste0("u0", i), P[i])    
          assign(paste0("u1", i), Q[i])
          assign(paste0("R", i), R[i])
           assign(paste0("NI", i), NI[i])
          
           assign(paste0("SDP", i), se1[i])
           assign(paste0("SDQ", i), se2[i])
    }
...
sofamean2 <- function(u0=1, u1=1, sd1=1, ratio=1 , sd2=1,   sig.level=0.05 , ni=.1, alternative=1){
...
A <-  sofamean2(u0=u01, u1=u11, sd=SDP1,   ratio=R1, sdr=SDP1/SDQ1, alternative=1, sig.level =A1 )

B <-  sofamean2(u0=u02, u1=u12, sd=SDP2,   ratio=R2, sdr=SDP2/SDQ2, alternative=1, sig.level =A2 )

C <-  sofamean2(u0=u03, u1=u13, sd=SDP3,   ratio=R3, sdr=SDP3/SDQ3, alternative=1, sig.level =A3 )
D <-  sofamean2(u0=u04, u1=u14, sd=SDP4,   ratio=R4, sdr=SDP4/SDQ4, alternative=1, sig.level =A4 )
E <-  sofamean2(u0=u05, u1=u15, sd=SDP5,   ratio=R5, sdr=SDP5/SDQ5, alternative=1, sig.level =A5 )
F <-  sofamean2(u0=u06, u1=u16, sd=SDP6,   ratio=R6, sdr=SDP6/SDQ6, alternative=1, sig.level =A6 )
