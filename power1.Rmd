...
```{r nopo input}
          ...
          value= "0.02823529, 0.02588235, 0.33764706, 0.36411765, 0.24411765,"),  )
          
            splitLayout(
          textInput("p3_nopo", div(h5(tags$span(style="color:blue", "Trt proportion(s) \U1D7B9 1..n"))), 
          value= " 0.02588235, 0.02823529, 0.33764706, 0.36411765, 0.24411765,"),  )
            
              splitLayout(
          textInput("p4_nopo", div(h5(tags$span(style="color:blue", "Trt proportion(s) \U1D7B9 1..n"))), 
          value= "0.02823529, 0.02588235, 0.36411765,  0.33764706,0.24411765,"),  )
...
#          p4        <-   ( p4_nopo )
 ...
 A <-   sig.level <-   (as.numeric(unlist(strsplit(input$alpha_nopo,","))))    
    ...
    #    d <- tidyr::expand_grid(p1 = p1, p2=p2, alpha=A, frac =Q, n=n , side=side)
...
#  
  ...
  for(i in 1: dim(L)[1]) {   
    ...
    # q <- (as.numeric(unlist(strsplit(L$p2[i],",")))) 
    # 
    # q1 <- (as.numeric(unlist(strsplit(L$p3[i],",")))) 
    # q2 <- (as.numeric(unlist(strsplit(L$p4[i],",")))) 
    # 
