...
```{r tab1}
          ...
          splitLayout(
          textInput("p1_z",    div(h5(tags$span(style="color:blue", "hypothesised proportion(s) \U1D7B9 1"))),    
          value= "0.2,0.25,0.35,0.2,0.25,0.35"),)
           
          splitLayout(
          textInput("p2_z",    div(h5(tags$span(style="color:blue", "hypothesised proportion(s) \U1D7B9 2"))),    
          value= "0.1,0.15,0.25,0.15,0.2,0.3"),)
          
          splitLayout(
          textInput("alpha_z", div(h5(tags$span(style="color:blue", "\U1D7AA (type I error)"))), 
          value= "0.05, 0.05,0.05, 0.05,0.05, 0.05"), 
                                       )
          splitLayout(
          textInput("ro_z",    div(h5(tags$span(style="color:blue", "ratio for unequal randomisation"))),   
          value= "1,1,1,1,1,1"),)                                
                                        
          sliderInput('x_range_z', 'Plot x-range', value = c(0,800),
                  min = 0, max = 5000,step=100,ticks=F)
          
          sliderInput('y_range_z', 'Plot y-range', value = c(0,1),
...
xx <- reactive({
    ...
    A <-   (as.numeric(unlist(strsplit(input$alpha_z,","))))    
    Az<-   (as.numeric(unlist(strsplit(input$p1_z,","))))    
    Aa<-   (as.numeric(unlist(strsplit(input$p2_z,","))))    
    Ab<-   (as.numeric(unlist(strsplit(input$ro_z,","))))    
   ...
   sample_sizes <- seq(10, input$x_range_z[2], by = 10)
   # 
   #---------------------------------------------------------------
...
renderPlot({
  ...
  #-----------------------------------------------------------------------
  skips <- ifelse(input$x_range_z[2] <= 1000, 50,
           ifelse(input$x_range_z[2] >  1000 & input$x_range_z[2] < 3000 ,200, 500))
  ...
  scale_y_continuous(breaks = seq(input$y_range_z[1], input$y_range_z[2], by = 0.1),
                     limits =   c(input$y_range_z[1], input$y_range_z[2])) +
  scale_x_continuous(breaks = seq(input$x_range_z[1], input$x_range_z[2], skips),
                     limits =   c(input$x_range_z[1], input$x_range_z[2])) +
    
  # scale_y_continuous(breaks = seq( y_range_z[1], y_range_z[2], by = 0.1),  
  #                  limits =   c( y_range_z[1],  y_range_z[2])) +
  #   scale_x_continuous(breaks = seq( x_range_Z[1],  x_range_z[2], skips), 
  #                      limits =   c( x_range_z[1],  x_range_z[2])) +  
...
```{r surv1}
       ...
       # hrs = c("2,1.5,0.9,2,1.5,0.9")
       # 
       #   alphas = c("0.05, 0.05,0.05, 0.05,0.05, 0.05")
       #                                 
       #    sidess = c("2,2,2,1,1,1")
       #                                   
       #     ros = c("1,2,3,1,2,3")                              
       #                                  
       #     x_ranges  = c(0,400) 
    ...
    #           A <-   (as.numeric(unlist(strsplit(alphas,","))))    
    # realHR <- Az<-   (as.numeric(unlist(strsplit(hrs,","))))    
    # Aa<-   (as.numeric(unlist(strsplit(sidess,","))))    
    # Ab<-   (as.numeric(unlist(strsplit(ros,","))))    
...
renderPlot({
 ...
 p <- p[complete.cases(p), ]
  ...
  skips <- ifelse(input$x_ranges[2] <= 1000, 50,
           ifelse(input$x_ranges[2] > 1000 & input$x_ranges[2] < 3000 ,200,  500))
  ...
  scale_x_continuous(breaks = seq(input$x_ranges[1], input$x_ranges[2], skips), 
                     limits =   c(input$x_ranges[1], input$x_ranges[2])) +
...
# proportional e↵ect of treatment across the scale of the ordinal outcome
...
Three arm trial (continuous and binary) with figure
===
...
```{r threearm12, eval=TRUE}
          ...
          # splitLayout(
          # textInput("N12",    div(h5(tags$span(style="color:blue", "sample size per arm"))),   
          # value= "40"),)        
          
  sliderInput('x_rangepo2', 'Plot x-range', value = c(0,300),
                  min = 0, max = 5000,step=100,ticks=F)
    ...
    three.arm.v2 <- function(u0, u1, u2, n,  sigma0, sigma1, sigma2, alpha1=0.05 ,  nSims=sims , out.var )
{
 ...
 threearmx <- reactive({
        ...
        link         <-   (as.numeric(unlist(strsplit(input$link2    ,","))))    # fraction
        sds        <-   (as.numeric(unlist(strsplit(input$sds2   ,","))))   
        ss        <-   (as.numeric(unlist(strsplit(input$ss12    ,","))))
      #  N         <-   (as.numeric(unlist(strsplit(input$N12     ,","))))
#---------------------------------------------------------------------------------------------------
   ...
   for( i in 1: length(n)) {
        ...
        names(result)  <-c("trt1 v Ctrl", "trt2 v Ctrl", "both v Ctrl", "conjunctive", "disjunctive", "at least 1", "mu1", "mu2", "mu3", "sigma0", 
"sigma1", "sigma2", "alpha", "out.var", "n")
...
"at least 1") )
  ...
  labs(y="Power", x = 'Total Sample Size', color = "Comparison")+
...
side <- ifelse(Ac==1,"one.sided",
                   ifelse(Ac==2,"two.sided", NA))
...
```{r nopo input}
          ...
          splitLayout(
          textInput("p1_nopo",    div(h5(tags$span(style="color:blue", "hyp. proportion(s) \U1D7B9 1..n"))),    
          value= "0.044, 0.019, 0.471, 0.317, 0.149"),)
           
          splitLayout(
          textInput("p2_nopo", div(h5(tags$span(style="color:blue", "odds ratios"))), 
          value= "0.044, 0.019, 0.471, 0.149, 0.317"),  )
          
          splitLayout(
          textInput("alpha_nopo", div(h5(tags$span(style="color:blue", "\U1D7AA (type I errors two-sided)"))), 
          value= "0.025"),   )
          
          splitLayout(
          textInput("side_nopo", div(h5(tags$span(style="color:blue", "\U1D7AA (type I errors two-sided)"))), 
          value= "1"),  )
          
          splitLayout(
          textInput("Q_nopo", 
          div(h5(tags$span(style="color:blue", "ratios for unequal randomisation \n (1 for equal randomisation)"))), 
          value= "0.5, 1"),)                                
                                        
          sliderInput('x_range_nopo', 'Plot x-range', value = c(0,800),
                  min = 0, max = 5000,step=100,ticks=F)
 ...
 A <-   sig.level <-   (as.numeric(unlist(strsplit(input$alpha_nopo,","))))    
       # p1         <-   (as.numeric(unlist(strsplit(input$p1_nopo,","))))    
        Q         <-   (as.numeric(unlist(strsplit(input$Q_nopo,"," ))))    # fraction
      #  p2        <-   (as.numeric(unlist(strsplit(input$p2_nopo,","))))   
    
         p1         <-   ( input$p1_nopo)    
         p2        <-   (input$p2_nopo )
     
        
          #   p1=c("0.044, 0.019, 0.471, 0.317, 0.149")
          # 
          # p2 = c("0.044, 0.019, 0.471, 0.149, 0.317")
          # 
          #  A = 0.025
          # 
          #  side_nopo=1
        ...
        d <- tidyr::expand_grid(p1 = p1, p2=p2, alpha=A, frac =Q, n=n , side=1)
    ...
    for(i in 1: dim(d)[1]) {   
    p <- (as.numeric(unlist(strsplit(d$p1[i],","))))  
    q <- (as.numeric(unlist(strsplit(d$p2[i],",")))) 
    
    p <- (as.numeric(unlist(strsplit(d$p1[i],","))))  
    q <- (as.numeric(unlist(strsplit(d$p2[i],",")))) 
   d$power[i] <- non_po_power(alpha = d$alpha[i], side=d$side[i], t = d$frac[i], p = p,  q =q,  N = n[i])
    #--------------------------------------------------------------------------------------------------------------------
 }
...
renderPlot({
 ...
 p <- p[complete.cases(p), ]
  ...
  all <- ggplot(p, aes(x = n, y = power, 
                  ...
                  )) + # # had hr here initally mis match with legend
  geom_line() +
