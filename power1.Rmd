...
3 arm trial (continuous and binary) simulation with figure
===
...
3 arm trial (continuous and binary) simulation
===
...
```{r bern}
          ...
          # splitLayout(
          # textInput("consist.", 
          # div(h5(tags$span(style="color:blue", "1/2 first time power reached/consistently above"))),value= "1"),  )
          # 
          sliderInput('x_ranges_bern', 'Plot x-range', value = c(0,100),
                  min = 0, max = 5000,step=100,ticks=F)
 ...
 berni <- reactive({
       ...
       # consist.   <-   (as.numeric(unlist(strsplit(input$consist.,  ","))))
#------------------------------------------------------------------------------------------------------------  
...
renderPlot({
  ...
  skips <- ifelse(input$x_ranges_bern[2] <= 500, 10,
           ifelse(input$x_ranges_bern[2] > 500 & input$x_ranges_bern[2] < 3000 ,200,  500))
